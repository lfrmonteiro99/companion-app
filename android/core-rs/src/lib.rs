...
//! ## Panic safety
//!
//! Rust panics must never unwind across an `extern "system"` frame into
//! the JVM — that is undefined behavior. Every entry point below wraps its
//! body in [`catch_unwind`] and returns a sentinel (null jstring / no-op)
...
/// Lazily build the shared tokio runtime. Returns `None` (rather than
/// panicking) if creation fails — e.g. thread/FD exhaustion under Android
/// resource limits — so the caller can report an error across JNI cleanly.
fn runtime() -> Option<&'static Runtime> {
    if RUNTIME.get().is_none() {
        match Runtime::new() {
            Ok(rt) => {
                // Ignore a lost race: another thread set it first, which
                // is fine — both runtimes are equivalent and the loser is
                // dropped.
...
/// Lock the shared state, recovering the guard if a previous holder
/// panicked (poison). Recovery is safe here because every handler fully
/// rebuilds the state it reads/writes; a poisoned-but-readable value is
/// preferable to panicking across the JNI boundary on every later call.
fn lock_state() -> MutexGuard<'static, Option<CoreState>> {
    state_slot().lock().unwrap_or_else(|e| e.into_inner())
}

/// Run a `jstring`-returning JNI handler body, catching any panic so it
/// can't unwind across the FFI boundary (UB). Returns a null jstring on
/// panic — Kotlin already has to null-check these returns.
fn guard_jstring(f: impl FnOnce() -> jstring) -> jstring {
    catch_unwind(AssertUnwindSafe(f)).unwrap_or_else(|_| {
        log::error!("panic caught at JNI boundary; returning null");
        std::ptr::null_mut()
    })
}

/// Run a unit-returning JNI handler body with the same panic guard.
fn guard_unit(f: impl FnOnce()) {
    if catch_unwind(AssertUnwindSafe(f)).is_err() {
        log::error!("panic caught at JNI boundary (ignored)");
    }
}
...
pub extern "system" fn Java_com_companion_awareness_CoreBridge_learnInterest<'local>(
    ...
    guard_unit(move || {
        let topic: String = match env.get_string(&topic) {
            Ok(s) => s.into(),
            Err(e) => {
                log::warn!("learnInterest: bad string: {e}");
                return;
            }
        };
        with_state_mut(|state| {
...
pub extern "system" fn Java_com_companion_awareness_CoreBridge_setExplicitInterests<'local>(
    ...
    guard_unit(move || {
        let len = match env.get_array_length(&items) {
            Ok(l) => l,
            Err(e) => {
                log::warn!("setExplicitInterests: bad array: {e}");
                return;
            }
        };
